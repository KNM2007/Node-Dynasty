  ...
  if (userAddress) {
    ...
    if (window.ethereum) {
      try {
        ...
        if (chainId === '0xaa36a7') domain = 'sepolia.etherscan.io'; // Sepolia
        else if (chainId === '0x89') domain = 'polygonscan.com'; // Polygon
        else if (chainId === '0x2105') domain = 'basescan.org'; // Base
        else if (chainId === '0x14a34') domain = 'sepolia.basescan.org'; // Base Sepolia
        else if (chainId === '0xa4b1') domain = 'arbiscan.io'; // Arbitrum
        else if (chainId === '0x38') domain = 'bscscan.com'; // BSC
        else if (chainId === '0x61') domain = 'testnet.bscscan.com'; // BSC Testnet
      } catch (e) {}
    }
    window.open(`https://${domain}/address/${userAddress}`, '_blank');
...
async function loadDashboardFallback() {
  ...
  const cidUpper = currentEmployee.companyId.toUpperCase();
  const rawHistory = localStorage.getItem('proofPayMockHistory');
  const allHistory = rawHistory ? JSON.parse(rawHistory) : {};
  const history = allHistory[cidUpper] || [];

  const totalClaimed = history.reduce((sum, entry) => sum + entry.amount, 0);
  el('balanceOut').textContent = totalClaimed + '.0 PPAY (Preview)';
  ...
  const lastClaim = history.length > 0 ? history[history.length - 1].timestamp : 0;
  const NINETY_DAYS = 90 * 24 * 60 * 60 * 1000;
  const canClaimTime = lastClaim + NINETY_DAYS;
  const now = Date.now();
  const isCooldown = now < canClaimTime;
  const eligibleAmount = estimatedScore + 10;

  btn.onclick = null; // Clear mock handler
  ...
  if (estimatedScore <= 50) {
    ...
    const nextDate = new Date(canClaimTime);
    stampEl.textContent = 'LOCKED';
    stampEl.className = 'stamp locked';
    detailEl.textContent = `You've already claimed this cycle. Next reward opens on ${nextDate.toLocaleDateString(undefined, { year: 'numeric', month: 'long', day: 'numeric' })} with a score of ${estimatedScore}.`;
    btn.disabled = true;
  } else {
    stampEl.textContent = 'AVAILABLE';
    stampEl.className = 'stamp available';
    detailEl.textContent = `You are eligible to claim ~${eligibleAmount} PPAY reward tokens.`;
    btn.disabled = false;

    btn.onclick = async () => {
      btn.disabled = true;
      btn.textContent = 'Claiming (Mock)…';
      await new Promise(r => setTimeout(r, 1000)); // simulate tx delay

      const updatedHistory = JSON.parse(localStorage.getItem('proofPayMockHistory') || '{}');
      if (!updatedHistory[cidUpper]) updatedHistory[cidUpper] = [];
      updatedHistory[cidUpper].push({ timestamp: Date.now(), amount: eligibleAmount, score: estimatedScore });
      localStorage.setItem('proofPayMockHistory', JSON.stringify(updatedHistory));
  ...
  if (history.length === 0) {
    list.innerHTML = '<div class="history-empty">No rewards claimed yet. (Demo Mode)</div>';
  } else {
    list.innerHTML = '';
    const sorted = [...history].sort((a, b) => b.timestamp - a.timestamp);
    sorted.forEach(row => {
      const date = new Date(row.timestamp).toLocaleDateString(undefined, { year: 'numeric', month: 'short', day: 'numeric' });
      const div = document.createElement('div');
      div.className = 'history-row';
      div.innerHTML =
        `<span class="history-date">${date} · score ${row.score}</span>` +
        `<span class="history-amt">+${row.amount} PPAY</span>`;
